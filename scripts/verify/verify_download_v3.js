...
(async () => {
  ...
  const c1 = await p1.evaluate(() => ({
    ...
    qr: !!document.querySelector('#dlGrid .dl-qr img'),
    sub: (document.querySelector('.dl-sub') || {}).textContent || '',
  }));
  chk('①三卡(桌面UA：win/mac 直链 + 安卓二维码)', c1.cards === 3 && c1.links.length === 2 && c1.qr, JSON.stringify(c1));
  chk('①win/mac 链接指 gitee release', c1.links.length === 2 && c1.links.every(u => u.includes('gitee.com/deskbud/version/releases/download')), c1.links.join(' | '));
  ...
  chk('②有did→仍直链无购买态', c2.links.length === 2 && c2.buyers === 0, JSON.stringify(c2));
  ...
  chk('③英文文案(Download free)', c3.btn.includes('Download free') && /free/i.test(c3.sub), JSON.stringify(c3));
  ...
  await p1.screenshot({ path: __dirname + '/../../outputs/download_free.png', fullPage: true });
  await b.close();
