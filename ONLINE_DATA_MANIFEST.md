...
| `data/catalog.json` | 旧目录源（详情页/PetsView 曾用，含 `#catalogData` 内联） | ~~网站~~ **2026-09-16 起网站已不再在线 fetch**（site.js 仅注释提及）；**pyside6 `CATALOG_URL` 仍在拉（兜底旧源）**；kotlin（待核实） | **在用·仅 pyside6 兜底** | 与 works.json 角色重叠；待 pyside6/kotlin 改拉 works.json 后退役入 `_local_backup/` |
| `data/buddies-snapshot.json` | 客户端目录增量真源（带顶层 payload sha256，供客户端增量判定） | pyside6 `SNAPSHOT_URL`（主源）；kotlin（据 pyside6 注释"已消费"，待 kotlin 侧代码核实）；**网站不消费** | **在用·仅客户端** | 由 `scripts/gen_buddies_snapshot.py` 从 `works.json` 派生 |
...
> ⚠️ **catalog.json / buddies-snapshot.json 退役条件**：网站已不再消费两者（2026-09-16 单源化）。退役前提是 **pyside6 去掉 `CATALOG_URL` 兜底、改拉 `works.json`，且 kotlin 同步**；二者都切换后，catalog.json 与 buddies-snapshot.json 方可移入 `_local_backup/`（本地备份，不部署）。在此之前 pyside6/kotlin 仍在拉，必须保持在线。
...
**待各端跟进（见协同板 [老曹→@pyside6]/[@kotlin]）**：
- pyside6：去掉 `CATALOG_URL`（catalog.json 兜底），改拉 `works.json`；`SNAPSHOT_URL`（buddies-snapshot.json）保留作客户端增量源。
- kotlin：核实其目录拉取来源，统一到 works.json（或保留 snapshot，视客户端增量需要）。
- 二者切换完成后，catalog.json + buddies-snapshot.json 移入 `_local_backup/`。
